...
function DataSampler:get(headSampling)
  ...
  elseif headSampling == 2 then -- sample score
    ...
    input, label = self:attentionSampling()
  end
...
--------------------------------------------------------------------------------
-- function: attention sampling
function DataSampler:attentionSampling()
  ...
  local mask = self:cropMask(ann, bboxInpSz, h, w, gSz)
  local att = torch.FloatTensor(#mask):zero()
  mask = mask:view(1, self.gSz, self.gSz)
  
  local box = self.maskApi.toBbox(self.maskApi.encode(mask))[1]
