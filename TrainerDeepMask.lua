...
function Trainer:__init(model, criterion, config)
  ...
  self.config = config
  self.model = model
  self.attNet = nn.Sequential():add(model.trunk):add(model.attentionBranch)
  ...
  self.lr = config.lr
  self.optimState ={}
  for k,v in pairs({'trunk','mask','score', 'att'}) do
    self.optimState[v] = {
  ...
  self.pa,self.ga = model.attentionBranch:getParameters()
...
function Trainer:test(epoch, dataloader)
  ...
  for n, sample in dataloader:run() do
    ...
    elseif sample.head == 2 then
      ...
      local outputs = self.scoreNet:forward(self.inputs)
      self.scoremeter:add(outputs, self.lables)
    end
    cutorch.synchronize()
