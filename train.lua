...
cmd:option('-datadir', 'data/images', 'data directory')
cmd:option('-seed', 1, 'manually set RNG seed')
cmd:option('-gpu', 5, 'gpu device')
...
cmd:option('-oSz', 40, 'output size')
cmd:option('-gSz', 160, 'ground truth size')
cmd:option('-shift', 16, 'shift jitter allowed')
cmd:option('-scale', .25, 'scale jitter allowed')
cmd:option('-hfreq', 0.5, 'mask&att/score head sampling frequency')
cmd:option('-mafreq', 0.5, 'mask/att head sampling frequency')
...
model = model or nn.DeepMask(config)
